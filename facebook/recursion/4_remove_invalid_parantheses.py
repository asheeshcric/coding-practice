    ...
    def remaining(self, string, index, left_count, right_count, expr, rem_count):
        ...
        if index == len(string):
            ...
            if current_char != "(" and current_char != ")":
                ...
                self.remaining(
                    string, index + 1, left_count, right_count, expr, rem_count
                )
                expr.pop()
            else:
                ...
                self.remaining(
                    string, index + 1, left_count, right_count, expr, rem_count + 1
                )
                ...
                if string[index] == "(":
                    self.remaining(
                        string, index + 1, left_count + 1, right_count, expr, rem_count
                    )
                    ...
                    self.remaining(
                        string, index + 1, left_count, right_count + 1, expr, rem_count
                    )
